...
const Meeting = () => {
    ...
    const [rooms, setRooms] = useState([
        { id: 1, roomName: 'Room A', x: 410, y: 40, meetingList: [] },
        { id: 2, roomName: 'Room B', x: 600, y: 150, meetingList: [] },
        { id: 3, roomName: 'Room C', x: 560, y: 300, meetingList: [] },
        { id: 4, roomName: 'Room D', x: 750, y: 50, meetingList: [] },
    ]);
    ...
    const changeInput = useCallback((e) => {
        ...
        setInput((prevInput) => {
            ...
            // 유효성 검사
            if (name === "startTime") {
                const startTime = moment(`${prevInput.date} ${value}`, "YYYY-MM-DD HH:mm");
                const now = moment(); // 현재 시각 계산
    
                // 현재 시각 이전인지 검사
                if (startTime.isBefore(now)) {
                    toast.error("시작 시간은 현재 시각 이후여야 합니다.");
                    return prevInput; // 이전 상태 유지
                }
                ...
                return updatedInput; // 종료 시간을 설정한 후, 추가 유효성 검사를 실행하지 않음
            }
    
            // 종료 시간 유효성 검사
            if (name === "finishTime") {
                const startTime = moment(`${prevInput.date} ${prevInput.startTime}`, "YYYY-MM-DD HH:mm");
                const finishTime = moment(`${prevInput.date} ${value}`, "YYYY-MM-DD HH:mm");
            ...
            return updatedInput; // 유효성 검사를 통과한 경우에만 상태 업데이트
        });
    }, [setInput]);
    ...
    useEffect(() => {
        // 컴포넌트가 마운트될 때만 로그인 사용자 이름을 기본값으로 설정
        if (userInfo.userName && !input.empId) {
            setInput(prevInput => ({
                ...prevInput,
                empId: userInfo.userName
            }));
        }
    }, [userInfo.userName, input.empId]);
...
<div className="col-md-8 col-sm-10">
        ...
        {/* 회의실 예약 설명 */}
               <h3>회의실예약</h3>
               <br></br><span style={{fontSize:"15px"}}>1.예약가능시간 08:00AM~18:00PM</span>
               <br></br><span style={{fontSize:"15px"}}>2.최소예약단위:30분</span>
               <br></br><span style={{fontSize:"15px"}}>3.최대예약시간:1시간</span>
               <br></br><span style={{fontSize:"15px"}}>4.시작시간 선택시 종료시간 자동으로 선택 되어있음(1시간)</span>
               <br></br><span style={{fontSize:"15px"}}>5.현재시각기준 내일까지만 예약가능</span>
               <br></br>
               <br></br><span style={{fontSize:"20px", fontWeight:"bold"}}>예약 수정</span>
               <br></br><span style={{fontSize:"15px"}}>1.예약수정은 방변경 불가능</span>
               <br></br><span style={{fontSize:"15px"}}>2.최소예약단위:30분</span>
               <br></br><span style={{fontSize:"15px"}}>3.최대예약시간:1시간</span>
               <br></br>
               <br></br><span style={{fontSize:"15px"}}>예약 종료 시간이 지나면 자동삭제</span>
                {rooms.map((room) => (
                    <div
                    key={room.id}
                    className={`room ${selectedRoom === room.roomName ? 'selected-room' : ''}`}
