...
const Exam06 = () => {
    ...
    // 검사 결과를 memo로 만들면 입력될 때마다 무조건 검사를 한다
    // 우리가 원하는건 입력이 끝나면 검사가 되는 것.. memo를 쓰면 안된다
    // memo 대신 stat를 쓰고 자동이 아니라 수동으로 변경하게 처리

    const [isIdValid, setIsIdValid] = useState(false);
    const [isPwValid, setIsPwValid] = useState(false);
    const [checkPw, setCheckPw] = useState(false);

    //Id를 수동으로 검사할 수 있는 함수를 생성
    const checkMemberId = useCallback(() => {
        const regex = /^[a-zA-Z\d@#$%^&+=!]{6,}$/;
        if (id.trim() === '') {
            setIsIdValid('');
        }
        setIsIdValid(regex.test(id) ? 'is-valid' : 'is-invalid');
    }, [id]);

    // 비밀번호 검사 함수
    const checkMemberPw = useCallback(() => {
        const regex = /^[a-z](?=.*[A-Z])(?=.*[0-9])(?=.*[@#$%^&+=!])[a-zA-Z0-9@#$%^&+=!]{8,16}$/;
        setIsPwValid(regex.test(pw) ? 'is-valid' : 'is-invalid');
    }, [pw]);
                        ...
                        <form action='' method='post' autoComplete='off'>
                            ...
                            <div className="row mt-4">
                                <div className='col'>
                                    ...
                                    <input type="password" className={`form-control ${isPwValid}`} value={pw} onBlur={e => checkMemberPw()} placeholder="비밀번호 입력" onChange={e => setPw(e.target.value)} />
                                    <div className="valid-feedback">
                                    </div>
                                    <div className="invalid-feedback">
